...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Object.keys(menuButtons).forEach(buttonId => {
        ...
        button.addEventListener('click', (event) => {
            ...
            Object.values(menuButtons).forEach(m => m.open = false); // Close all menus
            menu.open = !isOpen; // Toggle the clicked menu
        });
    });

    // Close all menus if clicking outside
    document.addEventListener('click', () => {
        Object.values(menuButtons).forEach(m => m.open = false);
    });
});
